...
const isActiveRoute = (routePattern) => {
    ...
    if (routePattern.includes('*')) {
        ...
        return currentUrl.startsWith(`/${baseRoute.replace('.', '/')}`)
    }
    ...
    const routeUrl = routeMap[routePattern]
    if (routeUrl) {
        return routePattern.includes('*') ? currentUrl.startsWith(routeUrl) : currentUrl === routeUrl
    }
...
watch(() => page.props.flash, (flash) => {
    ...
    if (flash.success) {
        showToastMessage('success', 'Success', flash.success)
    }
    if (flash.error) {
        showToastMessage('error', 'Error', flash.error)
    }
    if (flash.warning) {
        showToastMessage('warning', 'Warning', flash.warning)
    }
    if (flash.info) {
        showToastMessage('info', 'Information', flash.info)
    }
            ...
            <nav class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 transition-colors duration-300">
                    ...
                    <div class="flex justify-between h-16">
                        <div class="flex">
                            ...
                            <div class="shrink-0 flex items-center">
                                <Link href="/dashboard">
                                    <ApplicationLogo class="block h-9 w-auto fill-current text-gray-800 dark:text-gray-200 transition-colors duration-300" />
                                </Link>
                            </div>

                            <!-- Navigation Links -->
                            <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                                <NavLink href="/dashboard" :active="isActiveRoute('dashboard')">
                                    Dashboard
                                </NavLink>

                                <NavLink
                                    v-if="['Admin', 'Analyst'].includes($page.props.auth.user?.role)"
                                    href="/network-logs"
                                    :active="isActiveRoute('network-logs.*')"
                                >
                                    Network Logs
                                </NavLink>
                                ...
                                <NavLink
                                    ...
                                    href="/ml-models"
                                    :active="isActiveRoute('ml-models.*')"
                                >
                                    ML Models
                                </NavLink>
                            </div>
                        ...
                        <!-- Right Side: Theme, Notifications + Settings -->
                ...
                >
                    <div class="pt-2 pb-3 space-y-1">
                        <ResponsiveNavLink href="/dashboard" :active="isActiveRoute('dashboard')">
                            Dashboard
                        </ResponsiveNavLink>
                        <ResponsiveNavLink
                            v-if="['Admin', 'Analyst'].includes($page.props.auth.user?.role)"
                    ...
                    <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
                        ...
                        <div class="mt-3 space-y-1">
                            ...
                            <ResponsiveNavLink href="/notifications">Notifications</ResponsiveNavLink>
